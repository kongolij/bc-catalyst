...
runtime.registerComponent(GesShowFooterClient, {
  ...
  description: 'GES-ready footer with a build-from-scratch mode and fully editable content.',
  icon: 'navigation',
  props: {
    className: Style(),
    mode: Select({
    ...
    logo: Group({
      ...
      props: {
        ...
        fallbackText: TextInput({ label: 'Text when no image', defaultValue: 'Logo' }),
    ...
    copyright: Group({
      ...
      props: {
        ...
        includeCurrentYear: Checkbox({ label: 'Add current year automatically', defaultValue: true }),
        text: TextInput({ label: 'Copyright text', defaultValue: '' }),
        additionalLabel: TextInput({ label: 'Additional legal-link text', defaultValue: '' }),
